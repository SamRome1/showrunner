...
const k = (t: string) => ({ text: t, color: colors.textSecondary });
...
const LINES: CodeLine[] = [
  ...
  [{ text: '// never drawn by hand', color: palette.amber }],
];
...
export const AssetRule: React.FC = () => (
  <SceneWrapper gap={26}>
    <MonoLabel pill warm dot delay={0}>
      Official sources only
    </MonoLabel>
    <CodeBlock lines={LINES} delay={8} stagger={6} fontSize={18} filename="assets.json" highlight={[3]} highlightAt={100} />
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 18, width: '100%', justifyItems: 'center', marginTop: 6 }}>
      {MARKS.map((m, i) => (
        <LogoBadge key={m} src={`assets/${m}.svg`} size={156} delay={DOCK_AT + i * 4} highlight={0.35} />
      ))}
    </div>
  </SceneWrapper>
);
